...
namespace Graphics
{
    ...
    uint32_t TargetResolution = k1080p;
    ...
    uint32_t DebugZoom = kDebugZoomOff;
}
...
void Graphics::Resize(uint32_t width, uint32_t height)
{
    ...
    // Make native width == display width for now
    g_NativeWidth = g_DisplayWidth;
    g_NativeHeight = g_DisplayHeight;
...
void Graphics::Initialize(void)
{
    ...
    ASSERT_SUCCEEDED(dxgiFactory->CreateSwapChainForHwnd(g_CommandManager.GetCommandQueue(),Graphics::g_hWnd, &swapChainDesc, nullptr, nullptr, &s_SwapChain1));
#else // UWP
